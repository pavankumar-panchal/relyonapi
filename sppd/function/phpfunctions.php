...
$newconnection_rci = mysql_connect($dbhost_rci, $dbuser_rci, $dbpwd_rci) or die("Cannot connect to Mysql server host3");
...
function runmysqlquery_rci($query)
{
    ...
    //Connect to Database
    mysql_select_db($dbname,$newconnection_rci) or die("Cannot connect to database");
    set_time_limit(3600);
    //Run the query
    $result = mysql_query($query,$newconnection_rci) or die(" run Query Failed in Runquery function1.".$query); //;
...
function vieworgeneratedealerpdfinvoice($slno,$type)
{
   ...
   // $pdf->Image('../pdfbillgeneration/images/relyon-logo.png',130, 248, 65, 30, 'PNG');
	...
	else
	{
		...
		<td width="72%"><div align="center"><strong>Description</strong></div></td>
		<td width="9%"><div align="center"><strong>Quantity</strong></div></td>
	...
	while($fetch = mysql_fetch_array($result))
	{
		...
		for($i=0,$j=0;$i<count($descriptionsplit),$j<count($proquantitysplit);$i++,$j++)
		{
			...
			if($description <> '')
			{
				...
				{
					$grid .= '<td width="72%" style="text-align:left;">'.$descriptionline[1].'<br/>
				...
				$grid .= '<td width="9%"  style="text-align:right;" >'.$proquantitysplit[$j].'</td>';				
		...
		if($fetch['servicedescription'] <> '')
		{
			for($i=0; $i<$servicedescriptioncount; $i++)
			{
				...
				if($checkInvoicedate < $checkJuly) {
				$grid .= '<td width="72%" style="text-align:left;">'.$servicedescriptionline[1].'<br/><span style="font-size:+6" ><strong>Item Description</strong> : '.$itemdesvalue.' </span></td>';
				} 
			    else 
			    {
			    	if($checkInvoicedate1 < $checkMarch ||  $servicedescriptionline[1] == "Saral RDP")
						$grid .= '<td width="72%" style="text-align:left;">'.$servicedescriptionline[1].'<br/><span style="font-size:+6" ><strong>Item Description</strong> : '.$itemdesvalue.' </span> / <span style="font-size:+6" ><strong>SAC:</strong> 997331</span></td>';
						...
						$grid .= '<td width="72%" style="text-align:left;">'.$servicedescriptionline[1].'<br/><span style="font-size:+6" ><strong>Item Description</strong> : '.$itemdesvalue.' </span> / <span style="font-size:+6" ><strong>SAC:</strong> 998434</span></td>';
						...
						$grid .= '<td width="72%" style="text-align:left;">'.$servicedescriptionline[1].'<br/><span style="font-size:+6" ><strong>Item Description</strong> : '.$itemdesvalue.' </span> / <span style="font-size:+6" ><strong>SAC:</strong> 998313</span></td>';
					else
						$grid .= '<td width="72%" style="text-align:left;">'.$servicedescriptionline[1].'<br/><span style="font-size:+6" ><strong>Item Description</strong> : '.$itemdesvalue.' </span> / <span style="font-size:+6" ><strong>SAC:</strong> 999293</span></td>';

				}	
				$grid .= '<td  width="9%" style="text-align:right;" ></td>';
	...
	<span style="font-size:+7" ><strong>Purchase Type</strong> : '.$descriptionline[2].'&nbsp;/&nbsp;<strong>Usage Type</strong> :'.$descriptionline[3].'&nbsp;&nbsp;/ &nbsp;<strong>PIN Number : <font color="#000000">'.$descriptionline[4].'</font></strong> (<strong>Serial</strong> : '.$descriptionline[5].')</span><br/><span style="font-size:+6" ><strong>Product Description</strong> : '.$productdesvalue.' </span><span style="font-size:+6" > / <strong>SAC</strong> : 997331</span></td>';*/
		...
		if($fetch['servicedescription'] <> '')
		{
			for($i=0; $i<$servicedescriptioncount; $i++)
			{
			    ...
			    {
			    	if($checkInvoicedate1 < $checkMarch ||  $servicedescriptionline[1] == "Saral RDP" || $servicedescriptionline[1] == "Web Hosting - New")
						...
						|| $servicedescriptionline[1] == "Digital Signing" || $servicedescriptionline[1] == "Deployment Charges" || $servicedescriptionline[1] == "Saral Accounts Customization" || $servicedescriptionline[1] == "Employee Information Portal Updation" || $servicedescriptionline[1] == "SPP Customization Updation" || $servicedescriptionline[1] == "Web Hosting Updation" 
						|| $servicedescriptionline[1] == "Employee Information Portal Mobile" || $servicedescriptionline[1] == "Saral Billing Customization" || 
						$servicedescriptionline[1] == "Saral GST Cloud - V1 - (2020-21)" || $servicedescriptionline[1] == "SPP Cloud Payroll With ESS" || $servicedescriptionline[1] == "SPP Cloud Payroll With ESS - Gold" || $servicedescriptionline[1] == "SPP Cloud Payroll With ESS - Diamond" || $servicedescriptionline[1] == "SPP Cloud Payroll With ESS - Platinum" || $servicedescriptionline[1] == "SPP Cloud Payroll With ESS - Silver" || $servicedescriptionline[1] == "Saral PayPack Cloud" || $servicedescriptionline[1] == "Saral PayPack Cloud - Bronze" || $servicedescriptionline[1] == "Saral PayPack Cloud - Gold" || $servicedescriptionline[1] == "Saral PayPack Cloud - Silver" || $servicedescriptionline[1] == "SPP Customization - New") 
						...
						|| $servicedescriptionline[1] == "AMC Charges-Matrix-Non Comprehensive" || $servicedescriptionline[1] == "Employee Information Portal - AMC" || $servicedescriptionline[1] == "AMC Charges - Accounts" || $servicedescriptionline[1] == "AMC Charges - Billing" || $servicedescriptionline[1] == "AMC Charges - Add-on Module(ARE/AI/FM)" || $servicedescriptionline[1] == "AMC Charges - GST" || $servicedescriptionline[1] == "API CHARGES" || $servicedescriptionline[1] == "GSP CHARGES/API CALLS" || $servicedescriptionline[1] == "API Consumption" || $servicedescriptionline[1] == "API Integration -GST E-Invoicing" || $servicedescriptionline[1] == "Implementation - Taxation" || $servicedescriptionline[1] == "Support Charges - PayPack" || $servicedescriptionline[1] == "Support Charges - Taxation" || $servicedescriptionline[1] == "Support Charges - Accounts" || 
						$servicedescriptionline[1] == "AMC Charges - PayPack" || $servicedescriptionline[1] == "SPP Implementation" )
...
function addinvoicelinebreak($linecount)
{
	switch($linecount)
	{
		...
		case '4':
		{
			$linebreak = '<tr><td width="10%"><br/><br/><br/><br/></td>
			<td width="76%">&nbsp;</td>
			<td width="14%">&nbsp;</td></tr>';
		}
		break;
