...
// Note for this module to work properly first update new product code in inv_mas_product table 
// and then add new and updation price inv_relyon_price table
// this is no code change required to do here as yearly activity
...
$myfile = fopen("testlogs.txt", "a") or die("Unable to open file!");
fwrite($myfile, "\n". $currentURL);
fclose($myfile);
...
$customerid = decodevalue($_REQUEST['Qa1iio9']);
	...
	//$productcode = $_REQUEST['PRODCODE'];
    
	if(rslgetcookie('customerid')!="")
	{
		if(rslgetcookie('customerid')!=$customerid)
		  $cusid = $customerid;
		else
		  $cusid = rslgetcookie('customerid');
	}
	else
	{
	...
	$custquery = "select inv_mas_customer.slno ,inv_mas_customer.place,inv_mas_customer.gst_no,inv_mas_state.state_gst_code,inv_mas_state.statename,
	inv_mas_district.districtname,inv_mas_customer.businessname,inv_mas_customer.customerid,inv_mas_district.statecode 
	...
	$custquery2 = "select GROUP_CONCAT(emailid) as emailid,  GROUP_CONCAT(cell) as cell, GROUP_CONCAT(phone) as phone, GROUP_CONCAT(contactperson) as contactperson from 
	inv_contactdetails where customerid = '".$cusid."'";
	$resultfetch2 = runmysqlqueryfetch($custquery2);
	
	$businessname =$custfetch['businessname'];

    	//added on 4th Jan 2018
        	$gst_no = $custfetch['gst_no'];
        	$state_gst_code = $custfetch['state_gst_code'];
    	//ends	
	...
	inv_mas_product.year in ('".$yearcount[0]."','".$yearcount[1]."') and inv_mas_product.group in ('TDS','STO','SVI','SVH','XBRL','GST');";
	...
	while($fetch0=mysql_fetch_array($result0))
	{
			...
			$count_subgroup = mysql_num_rows($result1);
            if($count_subgroup > 0)
            {
				...
				left join inv_mas_product on inv_mas_product.productcode = inv_dealercard.productcode
				left join inv_mas_dealer on inv_mas_dealer.slno = inv_dealercard.dealerid where inv_mas_product.year in ('".$yearcount[0]."','".$yearcount[1]."') 
				...
				AND inv_customerproduct.cardid <> 0 AND inv_dealercard.cardid <> 0";
				...
				/*$query3 = "select count(*) as count from 
				inv_dealercard
				left join inv_mas_product on inv_mas_product.productcode = inv_dealercard.productcode
				where inv_mas_product.year = '".$currentyear."' and subgroup = '".$checksubgroup."' 
				and inv_dealercard.customerreference = ".$cusid ;
	...
	//$grid .= ' </table>';
	 ...
	 <td width="23%" class="td-border-grid2"  colspan="2">
	 <input type="hidden" id="custid" name="custid" value="'.$cusid.'">
	 ...
	 <input type="hidden" name="state_gst_code" id="state_gst_code" value="'.$state_gst_code.'">
	 </td>
	 </tr></table>';
?>
<!doctype html>
  ...
  <table width="100%" border="0" cellspacing="0" cellpadding="0">
      ...
      <table width="300px" border="0" align="left" cellpadding="0" cellspacing="0">
        <tr>
          ...
          <table width="270" border="0" cellpadding="4" cellspacing="0" class="dashboardprofilebox">
                              ...
                              <tr>
                                <td height="20px">GSTIN : <?php echo(gridtrim40($gst_no));?></td>
                              </tr>
                               <tr>
          ...
          <tr>
                      ...
                      <tr>
                       ...
                       <tr>
                           <td style="text-align: center;">
                            GSTIN : <input type="text" name="gst_no" id="gst_no" value="<?php 
                            if($gst_no == '') { echo "Not Registered Under GST"; } else { echo $gst_no; } 
                            ?>" required><br><br>
                      ...
                      <tr>
                        <td style="font-size:12px; padding-left:91px"><font color="#FF0000">* Additional 18% GST applicable</font></td>
                      </tr>
                      <tr>
                        <td><? echo($grid);?></td>
                      </tr>
    ...
    <!--<tr>
      <td><table width="100%" border="0" cellspacing="0" cellpadding="0" align="center">
          ...
          <tr>
            <td class="content-mid"><table width="70%" border="0" cellspacing="0" cellpadding="0" align="center">
                <tr>
                  <td><div align="center"><strong style="font-size:14px">Note : After clicking 'Proceed for Payment' Button don't do actions like  'refresh' or 'go Back' or 'go forward' or 'clicking links in the page'. In such cases the trasactions may end or may be lead to double payments.</strong></div></td>
                </tr>
              </table></td>
          </tr>
          <tr>
