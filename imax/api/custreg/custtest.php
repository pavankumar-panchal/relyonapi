...
$businessname = trim($_REQUEST['custBusinessName']); 
$custaddress = $_REQUEST['custAddress'];
$custplace = $_REQUEST['custPlace']; 
$custstate = $_REQUEST['custState'];
$district = $_REQUEST['custDistrict'];
$pincode = $_REQUEST['custPincode'];
$custname = $_REQUEST['custName'];
$custphone = $_REQUEST['custPhone'];
$custcellno = $_REQUEST['custCellNo'];
$custemailid = $_REQUEST['custEmailId'];
...
$attacheddate = datetimelocal('Y-m-d').' '.datetimelocal('H:i:s');
 ...
 if($count > 0)
 {
    ...
    //$query5 = 'select customerreference from inv_dealercard where cardid = '.$custcard;
    //$fetch5 =  runmysqlqueryfetch($query5);
    //$custref = $fetch5['customerreference'];
    if(empty($custref))
    {
        $fetch0 = runmysqlqueryfetch($query0);
        ...
        $logquery ="INSERT INTO inv_logs_login(userid,`date`,`time`,`type`,system,device,browser) VALUES('".$lastslno."','".datetimelocal('Y-m-d')."','".datetimelocal('H:i:s')."','Emudra_API_Test','".$_SERVER['REMOTE_ADDR']."','DESKTOP','".$_SERVER['HTTP_USER_AGENT']."')";
