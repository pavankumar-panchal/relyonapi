...
function mq(id)
{
	...
	for (var i=0;i < maxw;i++)
	{
		...
		//this.mqo.ary[i].style.zIndex = '100'; 
		this.mqo.appendChild(this.mqo.ary[i]);
	} 
	mqr.push(this.mqo);startstop(mqr,this.mqo);
} 
...
{
	var divisions = ['flashnewsholder','productsholder','messagesholder','rateitholder','supportholder'];
	
	for(i=0; i< divisions.length; i++)
	{
		if(divisions[i] == divid)
			$('#'+divisions[i]).fadeIn('slow');
		else
			$('#'+divisions[i]).hide();
	}
