...
<span style="font-size:+7" ><strong>Purchase Type</strong> : '.$purchasetype.'&nbsp;/&nbsp;<strong>Usage Type</strong> :'.$usagetype.'&nbsp;&nbsp;/ &nbsp;<strong>PIN Number </strong>: '.$fetch['pinno'].' (<strong>Serial</strong> : '.$fetch['cardno'].')</span></td>';
	 ...
	 $amountinwords = convert_number($result3['total']);
	...
	while($fetch = mysql_fetch_array($result))
	{
		...
		else
		{
			$sezremarks = '';
				...
				$kkcolumn='<tr><td width="56%" style="text-align:right"></td>
                                <td width="30%" style="text-align:right"><strong>KK Cess @ 0.5% </strong></td>
				<td width="14%" style="text-align:right;font-size:+5">'.formatnumber($KK_Cess_tax).$appendzero.'</td></tr>';
		...
		else
		{
		    if($gst_tax_date <= $invoicedate)
		    {
		        ...
		        if($fetch['cgst'] != '0' &&  $fetch['sgst'] != '0')
		        {
                	...
                	$gst_tax_column ='<tr><td  width="56%" style="text-align:right"></td>
                	<td  width="30%" style="text-align:right"><strong>CGST Tax @'.$cgst_tax_rate.'% </strong></td>
                	<td width="14%" style="text-align:right;font-size:+9">'.formatnumber($fetch['cgst']).$appendzero.'</td></tr>';
                	
                	$gst_tax_column .='<tr><td  width="56%" style="text-align:right"></td>
                	<td  width="30%" style="text-align:right"><strong>SGST Tax @'.$sgst_tax_rate.'% </strong></td>
                	<td width="14%" style="text-align:right;font-size:+9">'.formatnumber($fetch['sgst']).$appendzero.'</td></tr>';
		        }
		        else
		        {
		            $igst_tax_amount = roundnearestvalue($fetch['amount'] * ($igst_tax_rate/100));
                    ...
                    $gst_tax_column ='<tr><td  width="56%" style="text-align:right"></td>
                    <td  width="30%" style="text-align:right"><strong>IGST Tax @'.$igst_tax_rate.'% </strong></td>
                    <td width="14%" style="text-align:right;font-size:+9">'.formatnumber($fetch['igst']).$appendzero.'</td></tr>';
		        }
...
<td  width="14%" style="text-align:right"><img src="http://www.relyonsoft.com/imax/images/relyon-rupee-small.jpg" alt="Relyonsoft" width="8" height="8" border="0" align="absmiddle"  />&nbsp;&nbsp;'.formatnumber($fetch['netamount']).$appendzero.'</td> 
	...
	$addstring ="api/sppnewupdatepurchase";
...
function sendreceipt($receiptno,$type,$invoicenoformat)
{
	...
	$sel_receipt_customer = "select * from inv_invoicenumbers where invoiceno = '".$invoicenoformat."';";
	$resultfetches = runmysqlqueryfetch($sel_receipt_customer);

	$amount = $resultfetches['amount'];
	$netamount = $resultfetches['netamount'];
	$deduction = $resultfetches['deduction'];
	$tanno = $resultfetches['tanno'];

	$emailid = $resultfetches['emailid'];	
...
if(($deduction == '1') && ($tanno != ''))
{
  $receiptamount = round($netamount - ($amount*(.10)));
}

		$array[] = "##AMOUNT##%^%".$receiptamount.".00";
...
function sendreceiptmailer($receiptno,$invoicenoformat)
{
	$sel_receipt = "select * from inv_mas_receipt where slno = '".$receiptno."';";
	$resultfetch1 = runmysqlqueryfetch($sel_receipt);

	$invoiceno = $resultfetch1['invoiceno'];
	$invoiceamount = $resultfetch1['invoiceamount'];
	$receiptamount = $resultfetch1['receiptamount'];
	$customerreference = $resultfetch1['customerreference'];
	$createdby = $resultfetch1['createdby'];

	$sel_receipt_customer = "select * from inv_invoicenumbers where invoiceno = '".$invoicenoformat."';";
	$resultfetch2 = runmysqlqueryfetch($sel_receipt_customer);

	$businessname = $resultfetch2['businessname'];
	$contactperson = $resultfetch2['contactperson'];
	$emailid = $resultfetch2['emailid'];
	$deduction = $resultfetch2['deduction'];
	$tanno = $resultfetch2['tanno'];

	$amount = $resultfetch2['amount'];
	$netamount = $resultfetch2['netamount'];

	$sel_receipt_dealer = "select * from inv_mas_dealer where slno = '".$createdby."';";
	$resultfetch3 = runmysqlqueryfetch($sel_receipt_dealer);

	$dealer_businessname = $resultfetch3['businessname'];
	$dealer_emailid = $resultfetch3['emailid'];
	...
	$msg = file_get_contents("../mailcontents/tanconfirm.htm");
	$textmsg = file_get_contents("../mailcontents/tanconfirm.txt");
...
{
  $receiptamount = round($netamount - ($amount*(.10)));
  $array[] = "##INVOICENOFULL##%^%".$invoicenoformat;
}
	$array[] = "##AMOUNT##%^%".$receiptamount.".00";
	$array[] = "##CONTACTPERSON##%^%".$contactperson;
	$array[] = "##EMAILID##%^%".$emailid;
	...
	$dealeremailid = "webmaster@relyonsoft.com";
...
$filearray = array(array('../images/relyon-logo.jpg','inline','1234567890'),array('../images/relyon-rupee-small.jpg','inline','1234567892'));
...
/*$bccemailids = array('Bigmail' => 'bigmail@relyonsoft.com', 'Accounts'=> 'accounts@relyonsoft.com', 'Webmaster' => 'webmaster@relyonsoft.com', 'Relyonimax'=> 'relyonimax@gmail.com');*/
		...
		$subject = "Customer Tan No Mail ".$businessname. " for invoice ".$invoiceno ;
		$html = $msg;
		$text = $textmsg;
		$replyto = $ccemailids[$ccemailarray[0]];
		rslmail($fromname, $fromemail, $toarray, $subject, $text, $html,$ccarray,$bccarray,$filearray,$replyto);
...
function viewreceipt($receiptno,$type)
{
	...
	$deduction_pay = $fetchresult['deduction'];
	$tanno_pay = $fetchresult['tanno'];
	$amount_pay = $fetchresult['amount'];
	$netamount_pay = $fetchresult['netamount'];
...
$receiptamount_pay = formatnumber($result1['receiptamount']);
if(($deduction_pay == '1') && ($tanno_pay != ''))
{
  $receiptamount_pay = round($netamount_pay - ($amount_pay*(.10)));
}

	$array[] = "##AMOUNT##%^%".$receiptamount_pay.".00";

	$array[] = "##AMOUNTINWORDS##%^%".convert_number($receiptamount_pay);
