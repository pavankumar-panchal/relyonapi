...
left join inv_mas_branch on  inv_mas_branch.slno = inv_mas_dealer.branch where inv_mas_dealer.slno = '".$currentdealer."'");
	...
	while($carddetailsfetch = mysql_fetch_array($carddetailsresult))
	{
		...
		$description .= $slno.'$'.$carddetailsfetch['productname'].' - ('.$carddetailsfetch['year'].')'.'$'.$purchasetype.'$'.$usagetype.'$'.$carddetailsfetch['scratchnumber'].'$'.$carddetailsfetch['cardid'].'$'.$trans_description_split_vars[6];
		$k++;
		$descriptioncount++;
	}
	...
	$query666 = "UPDATE inv_invoicenumbers  SET customerid = '".cusidcombine($customerid)."', businessname = '".trim($company)."',contactperson = '".$contactpersonplit."', address = '".$invaddress."', place = '".$place."',pincode = '".$pincode."' ,emailid = '".$emailidplit."' , description = '".$description."' ,dealername = '".$dealername."', createddate = '".date('Y-m-d').' '.date('H:i:s')."',createdby = 'Webmaster', amount = '".$trans_amount."' , servicetax  = '".$trans_sevicetax."', sbtax  = '".$trans_sbtax."',kktax  = '".$trans_kktax."', netamount = '".$trans_netamount."' , phone  = '".$phonenumbersplit."' , cell = '".$cellnumbersplit."', customertype ='".$typename."' ,customercategory = '".$categoryname."' ,region = '".$regionname."' ,purchasetype = 'Product' ,dealerid = '".$dealerid."' ,products = '".$productcodearray."' ,productquantity =    '".trim($productquantity,',')."' ,pricingtype = 'default' ,createdbyid = '2' ,totalproductpricearray = '".$trans_totalproductpricearray."',actualproductpricearray = '".$trans_totalproductpricearray."' ,module = 'user_module' ,servicetype = '".$trans_servicetype."' ,serviceamount = '".$trans_serviceamount."' ,paymenttypeselected = 'paymentmadenow', itembriefdescription = '".$trans_itembriefdescription."' ,productbriefdescription = '".$trans_productbriefdescription."' ,paymentmode = '".$paymentmode."' ,stdcode = '".$invstdcode."' ,branch = '".$branchname."' ,amountinwords = '".$amountinwords."' ,remarks = '".$invoicepayremarks."' ,servicetaxdesc = '' ,invoiceheading = '".$invoiceheading."',servicedescription = '".$trans_servicedescription."',offerdescription = '' ,offerremarks = '' ,invoiceremarks = 'None' ,duedate = '',status = 'ACTIVE',regionid = '".$regionid."',branchid = '".$branchid."' where slno = '".$onlineinvoiceno."';";
	...
	#########  Mailing Starts -----------------------------------
		...
		$html = $msg;
		$text = $textmsg;
                $toarray_mail = array('samarsrivastva88@gmail.com');
