...
/* -------------------- Run a query to log database -------------------- (new)*/
...
function getpagelink($linkvalue)
{
  switch($linkvalue)
  {
    ...
    case 'addproductsnew': return '../main/addproductsnew.php'; break;
    case 'customusertest': return '../main/customusertest.php'; break;
    case 'customfilter': return '../main/customfilter.php'; break;
...
function getpagetitle($linkvalue)
{
  switch($linkvalue)
  {
    ...
    case 'addproductsnew': return 'Add Products New'; break;
    case 'customusertest': return 'Custom User Test'; break;
    case 'customfilter': return 'Custom Filter'; break;
...
function sendwelcomeemail($customerslno,$userid)
{
  ...
  if(($_SERVER['HTTP_HOST'] == "bhumika") || ($_SERVER['HTTP_HOST'] == "192.168.2.79"))
  {
    $bccemailids['archanaab'] ='samar.s@relyonsoft.com';
  }
  else
  {
    $bccemailids['support'] ='samar.s@relyonsoft.com';
    $bccemailids['info'] ='samar.s@relyonsoft.com';
    $bccemailids['bigmail'] ='samar.s@relyonsoft.com';
  ...
  $html = $msg;
  $text = $textmsg;
  $replyto = 'samar.s@relyonsoft.com';
  ...
  $bccmailid = 'samar.s@relyonsoft.com';
...
function sendregistrationemail($customerproductslno,$userid)
{
  ...
  if(($_SERVER['HTTP_HOST'] == "bhumika") || ($_SERVER['HTTP_HOST'] == "192.168.2.79"))
  {
    $bccemailids['rashmi'] ='samar.s@relyonsoft.com'; }
  else
  {
    $bccemailids['relyonimax'] ='relyonimax@gmail.com';
    $bccemailids['bigmail'] ='samar.s@relyonsoft.com';
  ...
  rslmail($fromname, $fromemail, $toarray, $subject, $text, $html,null,$bccarray,$filearray,$replyto);
  //Insert the mail forwarded details to the logs table
  $bccmailid = $bccids.','.'samar.s@relyonsoft.com'; 
  inserttologs($userid,$customerslno,$fromname,$fromemail,$emailid,null,$bccmailid ,$subject);    
...
function sendpaymentreqemail($customerslno,$table,$userid)
{
  ...
  {
    $bccemailids['relyonimax'] ='relyonimax@gmail.com';
    $bccemailids['accounts'] ='samar.s@relyonsoft.com';
    $bccemailids['bigmail'] ='samar.s@relyonsoft.com';
  }
...
left join inv_mas_product on inv_mas_product.productcode = inv_billdetail.productcode
 where inv_billdetail.cusbillnumber = '".$firstbillnumber."';";
  ...
  $grid .='<tr><td ><table width="100%" border="0" cellspacing="0" cellpadding="4" bordercolor="#CCCCCC" style="border:1px solid "><tr bgcolor="#CCCCCC"><td width="10%" style="text-align:left;"><div align="center"><strong>Sl No</strong></div></td><td width="66%"><div align="center"><strong>Description</strong></div></td><td width="24%"><div align="center"><strong>Amount</strong></div></td></tr></table><table>';
  $k = 0;
  $descriptioncount = 0;
  ...
  while($fetch = mysqli_fetch_array($result))
  {
    ...
    $grid .= '<td width="66%" style="text-align:left;">'.$fetch['productname'].' - ('.$financialyear[$k].')<br/><span style="font-size:+7" ><strong>Purchase Type</strong> : '.$purchasetype.'&nbsp;/&nbsp;<strong>Usage Type</strong> :'.$usagetype.'&nbsp;&nbsp;/ &nbsp;<strong>PIN Number :<font color="#FF3300"> '.$fetch['pinno'].'</font></strong> (<strong>Serial</strong> : '.$fetch['cardno'].')</span></td>';
    ...
    if($service <> '')
    {
      ...
      for($i=0; $i<$servicesplitcount; $i++)
      {
        ...
        $grid .= '<tr>';
        $grid .= '<td width="10%" style="text-align:centre;">'.$slno.'</td>';
        $grid .= '<td width="66%" style="text-align:left;">'.$servicesplit[$i].'</td>';
        $grid .= '<td  width="4%" style="text-align:right;" >'.$serviceamountsplit[$i].$appendzero.'</td>';
        $grid .= "</tr>";
   ...
   $grid .= '<tr><td colspan="2" style="text-align:right" width="86%"><strong>Total</strong></td><td  width="14%" style="text-align:right" valign="top">'.$result3['total'].$appendzero.'</td></tr><tr><td colspan="2" width="56%" style="text-align:left"><span style="font-size:11px" > '.$servicetaxdesc.'</span></td>  <td  width="30%" style="text-align:right"><strong>Service Tax @ 10.3%</strong></td> <td  width="14%" style="text-align:right">'.$result3['taxamount'].$appendzero.'</td></tr><tr>  <td  width="56%" style="text-align:right"><div align="left"><span style="font-size:11px" >E.&amp;O.E.</span></div></td>  <td  width="30%" style="text-align:right"><strong>Net Amount</strong></td><td  width="14%" style="text-align:right"><img src="../images/relyon-rupee-small.jpg" width="8" height="8" border="0" align="absmiddle"  />&nbsp;&nbsp;'.$result3['netamount'].$appendzero.'</td> </tr><tr><td colspan="3" style="text-align:left"><strong>Rupee In Words</strong>: '.$amountinwords.' only</td></tr>';
...
function smsactivationmail($slnoinserted,$userid)
{
  ...
  #########  Mailing Starts -----------------------------------
  if(($_SERVER['HTTP_HOST'] == "bhumika") || ($_SERVER['HTTP_HOST'] == "192.168.2.79"))
  $emailids['rashmi'] = 'samar.s@relyonsoft.com';
  else
  ...
  if(($_SERVER['HTTP_HOST'] == "bhumika") || ($_SERVER['HTTP_HOST'] == "192.168.2.79"))
  {
    $bccemailids['meghana'] ='samar.s@relyonsoft.com';
  }
  else
  {
    $bccemailids['relyonimax'] ='relyonimax@gmail.com';
    $bccemailids['bigmail'] ='samar.s@relyonsoft.com';
    $bccemailids['webmaster'] ='samar.s@relyonsoft.com';
  }
...
function vieworgeneratepdfinvoice111($slno,$type)
{
  ...
  //  echo($fetch['description']); exit;
    $grid .= '<tr><td colspan="2" style="text-align:right" width="86%"><strong>Total</strong></td><td  width="14%" style="text-align:right">'.$fetch['amount'].$appendzero.'</td></tr><tr><td  width="56%" style="text-align:left"><span style="font-size:11px" >'.$fetch['servicetaxdesc'].$appendzero.' </span></td><td  width="30%" style="text-align:right"><strong>Service Tax @ 10.3%</strong></td><td  width="14%" style="text-align:right">'.$fetch['servicetax'].$appendzero.'</td></tr><tr><td  width="56%" style="text-align:right"><div align="left"><span style="font-size:11px" >E.&amp;O.E.</span></div></td><td  width="30%" style="text-align:right"><strong>Net Amount</strong></td><td  width="14%" style="text-align:right"><img src="../images/relyon-rupee-small.jpg" width="8" height="8" border="0" align="absmiddle"  />&nbsp;&nbsp;'.$fetch['netamount'].$appendzero.'</td> </tr><tr><td colspan="3" style="text-align:left"><strong>Rupee In Words</strong>: '.$fetch['amountinwords'].' only</td></tr>';
    }
  ...
  while($fetch = mysqli_fetch_array($result))
  {
    ...
    for($i=0;$i<count($descriptionsplit);$i++)
    {
      ...
      if($fetch['purchasetype'] == 'SMS')
      {
        $grid .= '<tr>';
        $grid .= '<td width="10%" style="text-align:centre;">'.$descriptionline[0].'</td>';
        $grid .= '<td width="66%" style="text-align:left;">'.$descriptionline[1].'</td>';
        $grid .= '<td  width="24%" style="text-align:right;" >'.$descriptionline[2].'</td>';
        $grid .= "</tr>";
    ...
    $grid .= '<tr><td  width="56%" style="text-align:left"><span style="font-size:11px;color:#FF0000" >'.$statusremarks.'</span></td><td  width="30%" style="text-align:right"><strong>Total</strong></td><td  width="14%" style="text-align:right">'.$fetch['amount'].$appendzero.'</td></tr><tr><td  width="56%" style="text-align:left"><span style="font-size:11px" >'.$fetch['servicetaxdesc'].' </span></td><td  width="30%" style="text-align:right"><strong>Service Tax @ 10.3%</strong></td><td  width="14%" style="text-align:right">'.$fetch['servicetax'].$appendzero.'</td></tr><tr><td  width="56%" style="text-align:right"><div align="left"><span style="font-size:11px" >E.&amp;O.E.</span></div></td><td  width="30%" style="text-align:right"><strong>Net Amount</strong></td><td  width="14%" style="text-align:right"><img src="../images/relyon-rupee-small.jpg" width="8" height="8" border="0" align="absmiddle"  />&nbsp;&nbsp;'.$fetch['netamount'].$appendzero.'</td> </tr><tr><td colspan="3" style="text-align:left"><strong>Rupee In Words</strong>: '.$fetch['amountinwords'].' only</td></tr>';
  //  echo($grid1); exit;
  //  $grid .= '<tr><td colspan="2" style="text-align:right" width="86%"><strong>Total</strong></td><td  width="14%" style="text-align:right">'.$fetch['amount'].$appendzero.'</td></tr><tr><td  width="56%" style="text-align:left"><span style="font-size:11px" >'.$fetch['servicetaxdesc'].$appendzero.' </span></td><td  width="30%" style="text-align:right"><strong>Service Tax @ 10.3%</strong></td><td  width="14%" style="text-align:right">'.$fetch['servicetax'].$appendzero.'</td></tr><tr><td  width="56%" style="text-align:right"><div align="left"><span style="font-size:11px" >E.&amp;O.E.</span></div></td><td  width="30%" style="text-align:right"><strong>Net Amount</strong></td><td  width="14%" style="text-align:right"><img src="../images/relyon-rupee-small.jpg" width="8" height="8" border="0" align="absmiddle"  />&nbsp;&nbsp;'.$fetch['netamount'].$appendzero.'</td> </tr><tr><td colspan="3" style="text-align:left"><strong>Rupee In Words</strong>: '.$fetch['amountinwords'].' only</td></tr>';
    }
...
function vieworgeneratepdfinvoice($slno,$type)
{
  ...
  while($fetch = mysqli_fetch_array($result))
  {
    ...
    if($fetch['servicedescription'] <> '')
    {
      for($i=0; $i<$servicedescriptioncount; $i++)
      {
        ...
        $grid .= '<tr>';
        $grid .= '<td width="10%" style="text-align:centre;">'.$servicedescriptionline[0].'</td>';
        $grid .= '<td width="76%" style="text-align:left;">'.$servicedescriptionline[1].'<br/><span style="font-size:11px" ><strong>Item Description</strong> : '.$itemdesvalue.' </span></td>';
    ...
    if($fetch['seztaxtype'] == 'yes')
    {
      ...
      else
      {
        ...
        <td  width="14%" style="text-align:right"><span style="font-size:12px" >'.$totalservicetax1.'</span>
        </td></tr>';
  if($KK_Cess_date < $invoicedate)
    ...
    else
    {
      ...
      else
      {
        ...
        if($KK_Cess_date < $invoicedate)
        {
           ...
           $kkcolumn='<tr><td  width="56%" style="text-align:right"></td><td  width="30%" style="text-align:right"><strong>KK Cess @ 0.5% </strong></td><td width="14%" style="text-align:right;font-size:10px">'.formatnumber($KK_Cess_tax).$appendzero.'</td></tr>';
        }
      }
    ...
    $grid .= '<tr>
    <td  width="56%" style="text-align:left"><span style="font-size:11px" >'.$fetch['servicetaxdesc'].' </span></td>
    ...
    <tr>
    <td  width="56%" style="text-align:left"><span style="font-size:11px;color:#FF0000" >'.$sezremarks.'</span><span style="font-size:11px;color:#FF0000" >'.$statusremarks.'</span></td>
    <td  width="30%" style="text-align:right"><span style="font-size:12px" ><strong>'.$servicetaxname.'</strong></span></td><td width="14%" style="text-align:right"><span style="font-size:12px" >'.$totalservicetax.'</span></td></tr>'.$sbcolumn .$kkcolumn;



$grid .= '<tr>
<td  width="56%" style="text-align:right"><div align="left"><span style="font-size:11px" >E.&amp;O.E.</span></div></td>
    ...
    for($i=0;$i<count($descriptionsplit);$i++)
    {
      ...
      {*/
        ...
        if($description <> '')
        {
          ...
          $grid .= '<td width="76%" style="text-align:left;">'.$descriptionline[1].'<br/>
      <span style="font-size:+7" ><strong>Purchase Type</strong> : '.$updationtype.'&nbsp;/&nbsp;<strong>Usage Type</strong> :'.$descriptionline[3].'&nbsp;&nbsp;</span><br/><span style="font-size:11px" ><strong>Product Description</strong> : '.$productdesvalue.' </span></td>';
          $grid .= '<td  width="14%" style="text-align:right;" >'.formatnumber($descriptionline[6]).$appendzero.'</td>';
          $grid .= "</tr>";
                                     $countt++;
        }
  ...
  if($KK_Cess_date < $invoicedate)
    ...
    else
    {
      ...
      else
      {
        ...
        {
                 $KK_Cess_tax = roundnearestvalue($fetch['amount'] * 0.005);
           $kkcolumn='<tr><td  width="56%" style="text-align:right"></td><td  width="30%" style="text-align:right"><strong>KK Cess @ 0.5% </strong></td><td width="14%" style="text-align:right;font-size:12px">'.formatnumber($KK_Cess_tax).$appendzero.'</td></tr>';
        }
      }
...
$grid .= '<tr>
<td  width="56%" style="text-align:right"><div align="left"><span style="font-size:11px" ></span></div></td>
<td  width="30%" style="text-align:right"><strong>Total</strong></td>
<td  width="14%" style="text-align:right"><img src="../images/relyon-rupee-small.jpg" width="8" height="8" border="0" align="absmiddle"  />&nbsp;&nbsp;'.formatnumber($fetch['netamount'] ).$appendzero.'</td> 
</tr><tr><td colspan="3" style="text-align:left"><strong>Rupee In Words</strong>: '.convert_number($fetch['netamount']).' only</td></tr>';
...
function sendpurchasesummaryemail($dealerid,$recordreferencestring)
{
  ...
  for($i = 0; $i < $bccemailcount; $i++)
  {
    if(checkemailaddress($bccemailarray[$i]))
    {
      if($bccemailarray[$i]!= "samar.s@relyonsoft.com")
          {
        if($i == 0)
...
;
  ...
  if($filebasename <> '')
  {
    ...
    {
      $bccemailids = array('Bigmail' => 'samar.s@relyonsoft.com');
    }
    $bccarray = $bccemailids;
    $msg = replacemailvariable($msg,$array);
    $textmsg = replacemailvariable($textmsg,$array);
...
function sendreceipt($receiptno,$type)
{
  ...
  if($filebasename <> '')
  {
    ...
    {
      $bccemailids = array('Accounts'=> 'samar.s@relyonsoft.com');
    }
    $bccarray = $bccemailids;
    $msg = replacemailvariable($msg,$array);
    $textmsg = replacemailvariable($textmsg,$array);
...
function sendimplementationmail($slno,$customerreference,$dealerid,$userid)
{
  ...
  {
    unset($emailids);
    $emailids[] = 'samar.s@relyonsoft.com';
  }
  ...
  if(($_SERVER['HTTP_HOST'] == "bhumika") || ($_SERVER['HTTP_HOST'] == "192.168.2.79"))
  {
    $resultemailid = 'samar.s@relyonsoft.com';
  }
  else
  {
  ...
  if(($_SERVER['HTTP_HOST'] == "bhumika") || ($_SERVER['HTTP_HOST'] == "192.168.2.79"))
  {
    $bccemailids['archana'] ='samar.s@relyonsoft.com';
  }
  else
  {
    $bccemailids['bigmail'] ='samar.s@relyonsoft.com';
...
function sendupdateimpmail($slno,$dealerid,$userid)
{
  ...
  if(($_SERVER['HTTP_HOST'] == "bhumika") || ($_SERVER['HTTP_HOST'] == "192.168.2.79"))
  {
    $bccemailids['Bhumika'] ='samar.s@relyonsoft.com';
  }
  else
  {
    $bccemailids['bigmail'] ='samar.s@relyonsoft.com';
...
function sendbranchappovedmail($lastslno,$dealerid,$userid)
{
  ...
  if(($_SERVER['HTTP_HOST'] == "bhumika") || ($_SERVER['HTTP_HOST'] == "192.168.2.79") )
  {
    $branchemailid = 'samar.s@relyonsoft.com';
    $coordinatoremaild = '';
    $branchuseremailid = '';
  }
  else
...
function sendsurrendermail()
{
  ...
  if($_SERVER['HTTP_HOST'] == "bhumika" || $_SERVER['HTTP_HOST'] == "192.168.2.79")
  {
    ...
    {
      $ccmymails[$businessname] ='samar.s@relyonsoft.com';
    }
  }
  else
  { 
  ...
  {
    #$bccemailids['Bhavesh'] ='samar.s@relyonsoft.com';
    $bccemailids['Webmaster'] ='samar.s@relyonsoft.com';
  }
...
function transferpinmail()
{
  ...
  if($_SERVER['HTTP_HOST'] == "bhumika" || $_SERVER['HTTP_HOST'] == "192.168.2.79")
  {
    //$ccmymails['Bhumika'] ='samar.s@relyonsoft.com';
    $ccmymails = NULL;
  }
  else
  { 
...
function mailtocustomers($customer_id)
{
         $customer_id1=cusidcombine($customer_id);
         //echo $customer_id1;
  $query = "SELECT  emailid1, businessname, place from `inv_mas_customer` WHERE customerid=$customer_id";
  ...
  $place = ($place == '')?'Not Available':$place;
        $amount = ($amount == '')?'Not Available':$amount;
  ...
  $grid="<table border='1px solid black' style='border-collapse: collapse;' width='100%'>";
                 ...
                 for($i=0;$i<count($descriptionsplit);$i++)
    {
      ...
      <span style="font-size:+7" ><strong>Purchase Type</strong> : '.$descriptionline[2].'&nbsp;/&nbsp;<strong>Usage Type</strong> :'.$descriptionline[3].'&nbsp;&nbsp;<br/><span style="font-size:11px" ><strong>Product Description</strong> : '.$productdesvalue.' </span></td>';
    ...
    if($fetch1['servicedescription'] <> '')
    {
      for($i=0; $i<$servicedescriptioncount; $i++)
      {
        ...
        $grid .= '<tr>';
        $grid .= '<td width="10%" style="text-align:centre;">'.$count.'</td>';
        $grid .= '<td width="66%" style="text-align:left;">'.$servicedescriptionline[1].'<br/><span style="font-size:11px" ><strong>Item Description</strong> : '.$itemdesvalue.' </span></td>';
    ...
    if($fetch1['seztaxtype'] == 'yes')
    {
      ...
      else
      {
        ...
        $sbcolumn = '<tr><td  width="56%" style="text-align:left">&nbsp;</td>
        <td  width="30%" style="text-align:right"><strong>'.$servicetaxname1.'</strong></td>
        <td  width="8%" style="text-align:right"><span style="font-size:12px" >'.$totalservicetax1.'</span>
        </td></tr>';
  if($KK_Cess_date < $invoicedate)
    {
      $kkcolumn = '<tr><td  width="56%" style="text-align:left">&nbsp;</td>
      <td  width="30%" style="text-align:right"><strong>'.$servicetaxname2.'</strong></td>
      <td  width="8%" style="text-align:right!important;"><span style="font-size:12px" >'.$totalservicetax1.'</span>
        </td></tr>';
    }
      }
    }
    else
    {
      ...
      {
        $servicetax1 = roundnearestvalue($fetch1['servicetax']);
        $servicetaxname = 'Service Tax @ 14%';
        $totalservicetax = formatnumber($servicetax1).$appendzero;
      }
      else
      {
        $servicetax1 = roundnearestvalue($fetch1['servicetax'] );
        $servicetax2 = roundnearestvalue($fetch1['sbtax']);
        $servicetaxname = 'Service Tax @ 14%';
        $servicetaxname1 = 'SB Cess @ 0.5%';
        $totalservicetax = formatnumber($servicetax1).$appendzero;
        $totalservicetax1 = formatnumber($servicetax2).$appendzero;
        ...
        if($KK_Cess_date < $invoicedate)
        {
                 $KK_Cess_tax = roundnearestvalue($fetch1['kktax'] );
           $kkcolumn='<tr><td  width="56%" style="text-align:right"></td><td  width="30%" style="text-align:right"><strong>KK Cess @ 0.5% </strong></td><td width="8%" style="text-align:right;font-size:10px">'.formatnumber($KK_Cess_tax).$appendzero.'</td></tr>';
        }
      }
    ...
    //echo($servicetax1.'#'.$servicetax2.'#'.$servicetax3); exit;
    $grid .= '<tr>
    <td  width="56%" style="text-align:left"><span style="font-size:11px" >'.$fetch1['servicetaxdesc'].' </span></td>
    ...
    <tr>
    <td  width="56%" style="text-align:left"><span style="font-size:11px;color:#FF0000" >'.$sezremarks.'</span><span style="font-size:11px;color:#FF0000" >'.$statusremarks.'</span></td>
    <td  width="30%" style="text-align:right"><span style="font-size:12px" ><strong>'.$servicetaxname.'</strong></span></td><td width="8%" style="text-align:right"><span style="font-size:12px" >'.$totalservicetax.'</span></td></tr>'.$sbcolumn .$kkcolumn;
...
<td  width="8%" style="text-align:right"><img src="../images/relyon-rupee-small.jpg" width="8" height="8" border="0" align="absmiddle"  />&nbsp;&nbsp;'.formatnumber($fetch1['netamount'] ).$appendzero.'</td> 
   ...
   $grid.='</td></tr></table></tr><tr><td><center><a href="http://imax.relyonsoft.com/sppnewupdatepurchase/index.php?Qa1iio9='.$payslno.'&AsWrIo='.$payproductcode.'" target="_blank" class="myButton">Pay Now</a></center></td></tr>'; 
...
//$toarray = $cus_emailid;
  ...
  $msg = replacemailvariable($msg,$array);
  $textmsg = replacemailvariable($textmsg,$array);
  $subject = 'Pro-forma Invoice,'.$product;
  $html = $msg;
  $text = $textmsg;
  $replyto = 'samar.s@relyonsoft.com';
  rslmail($fromname, $fromemail, $toarray, $subject, $text, $html,null,$bccarray,$filearray,$replyto);
  //Insert the mail forwarded details to the logs table
  $bccmailid = 'rakesh.sahoo@relyonsoft.com';
  inserttologs($userid,$customerslno,$fromname,$fromemail,$emailid,null,$bccmailid,$subject);
...
function mailtopinvcustomers($p_slno)
{
  ...
  $grid="<tr><td colspan='3'><table width='100%' border='1px solid black' style='border-collapse: collapse;' >";
       $product='';
        $description = $fetch1['description'];
    $productbriefdescription = $fetch1['productbriefdescription'];
    $productbriefdescriptionsplit = explode('#',$productbriefdescription);
                 ...
                 for($i=0;$i<count($descriptionsplit);$i++)
    {
      ...
      if($productbriefdescription <> '')
         ...
         if($descriptionline[6]>0)
        {
          ...
          $grid .= '<tr>';
          $grid .= '<td width="10%" style="text-align:centre;">'.$count.'</td>';
          $grid .= '<td width="66%" style="text-align:left;">'.$descriptionline[1].'<br/>
      <span style="font-size:12px;" ><strong>Purchase Type</strong> : '.$descriptionline[2].'&nbsp;/&nbsp;<strong>Usage Type</strong> :'.$descriptionline[3].'&nbsp;&nbsp;<br/><span style="font-size:12px" ><strong>Product Description</strong> : '.$productdesvalue.' </span></td>';
          $grid .= '<td  width="8%" style="text-align:right;" >'.formatnumber($descriptionline[6]).$appendzero.'</td>';
          $grid .= "</tr>";$incno++;$count++;
        }
        }
    ...
    if($fetch1['servicedescription'] <> '')
    {
      for($i=0; $i<$servicedescriptioncount; $i++)
      {
        ...
        $grid .= '<tr>';
        $grid .= '<td width="10%" style="text-align:centre;">'.$count.'</td>';
        $grid .= '<td width="66%" style="text-align:left;">'.$servicedescriptionline[1].'<br/><span style="font-size:12px" ><strong>Item Description</strong> : '.$itemdesvalue.' </span></td>';
        $grid .= '<td  width="8%" style="text-align:right;" >'.formatnumber($servicedescriptionline[2]).$appendzero.'</td>';
        $grid .= "</tr>";
                       $count++;
            }
    ...
    if($description <> '')
    {
     ...
     //$grid .= '<table width="100%" style="border-collapse: collapse;" border="1px solid black">';
      $descriptionlinecount = 1;
    }
    if($fetch1['description'] == '')
      $offerdescriptioncount = 0;
    ...
    if($fetch1['seztaxtype'] == 'yes')
    {
      ...
      else
      {
        ...
        $sbcolumn = '<tr><td  width="10%" style="text-align:left">&nbsp;</td>
        <td  width="66%%" style="text-align:right"><strong>'.$servicetaxname1.'</strong></td>
        <td  width="8%" style="text-align:right"><span style="font-size:12px;" >'.$totalservicetax1.'</span>
        </td></tr>';
  if($KK_Cess_date < $invoicedate)
    {
      $kkcolumn = '<tr><td  width="10%" style="text-align:left">&nbsp;</td>
      <td  width="66%" style="text-align:right"><strong>'.$servicetaxname2.'</strong></td>
      <td  width="8%" style="text-align:right!important;"><span style="font-size:12px;" >'.$totalservicetax1.'</span>
        </td></tr>';
    }
      }
    ...
    else
    {
      ...
      else
      {
        ...
        {
                 $KK_Cess_tax = roundnearestvalue($fetch1['kktax'] );
           $kkcolumn='<tr><td  width="10%" style="text-align:right"></td><td  width="66%" style="text-align:right"><strong>KK Cess @ 0.5% </strong></td><td width="8%" style="text-align:right;font-size:12px">'.formatnumber($KK_Cess_tax).$appendzero.'</td></tr>';
        }
      }
    ...
    <td  width="66%" style="text-align:right"><strong>Net Amount</strong></td>
    <td  width="8%" style="text-align:right">'.formatnumber($fetch1['amount']).$appendzero.'</td></tr>
    <tr>
    <td  width="10%" style="text-align:left"><span style="font-size:12px;color:#FF0000" >'.$sezremarks.'</span><span style="font-size:12px;color:#FF0000" >'.$statusremarks.'</span></td>
    <td  width="66%" style="text-align:right"><span style="font-size:12px" ><strong>'.$servicetaxname.'</strong></span></td><td width="8%" style="text-align:right"><span style="font-size:12px" >'.$totalservicetax.'</span></td></tr>'.$sbcolumn .$kkcolumn;



$grid .= '<tr>
<td  width="10%%" style="text-align:right"><div align="left"></div></td>
<td  width="66%" style="text-align:right"><strong>Total</strong></td>
<td  width="8%" style="text-align:right"><img src="../images/relyon-rupee-small.jpg" width="8" height="8" border="0" align="absmiddle"  />&nbsp;&nbsp;'.formatnumber($fetch1['netamount'] ).$appendzero.'</td> 
</tr><tr><td colspan="3" style="text-align:left"><strong>Rupee In Words</strong>: '.convert_number($fetch1['netamount']).' only</td></tr></td></tr>';
  

   $grid.='</td></tr></tr><tr><td></td></tr></table>'; 

$grid.='<center><a href="http://imax.relyonsoft.com/sppnewupdatepurchase/indexcopy.php?Qa1iio9='.$pinv_cus_id.'&AsWrIo='.$p_slno.'" target="_blank" class="myButton">Pay Now</a></center>';
$msg.=$grid;
$msg1 = file_get_contents("../mailcontents/mailcustomer1.htm");
$msg=$msg.$msg1;
...
function scratchcombine($scratchnumber)
{
  $result1 = substr($scratchnumber,0,4);
  $result2 = substr($scratchnumber,4,4);
  $result3 = substr($scratchnumber,8,4);
