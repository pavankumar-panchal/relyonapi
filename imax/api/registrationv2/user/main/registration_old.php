<?php
	...
	logdetailsregister2($CUSTID,$REGTYPE,$PID,$UNAME,$DWP,$PIN,$HDDID,$ETHID,$UDCID,$CMPNAME,$CMPID,$LGNTYPE);
...
$q1 = "select cardid from inv_mas_scratchcard where scratchnumber='".$PIN."' and attached = 'yes' and cancelled = 'no' and blocked = 'no'";
	...
	{ 
		...
		echo $output;
			exit;*/
		//exit;
		$q10="select * from inv_customerproduct where slno=".$customerproductslno;
		$r10 = runmysqlquery($q10);
		...
		if (matchHWComponents($HDDID, $HDDIDReturned) || matchHWComponents($ETHID, $ETHIDReturned))
		{
			...
			if($REQTYPE == "1" || $REQTYPE == "3")
			{
				...
				$output=$output .  "<PAYMENTSTATUS>1</PAYMENTSTATUS>";
				$output=$output .  "</LICENSE>";
				$output=$output .  "</ROOT>";
			...
			{
				$query11 = "select * FROM inv_customerproduct Where slno = ". $customerproductslno;
				$result11 = runmysqlquery($query11);
