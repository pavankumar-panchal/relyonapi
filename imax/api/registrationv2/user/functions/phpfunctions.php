...
left join inv_mas_product on inv_mas_product.productcode = inv_billdetail.productcode
 where inv_billdetail.cusbillnumber = '".$firstbillnumber."';";
	...
	$addstring ="/user";
	if($_SERVER['HTTP_HOST'] == "rashmihk" || $_SERVER['HTTP_HOST'] == "meghanab" || $_SERVER['HTTP_HOST'] == "archanaab"  || $_SERVER['HTTP_HOST'] == "localhost")
		$addstring = "/saralimax-user";
		$filepath = $_SERVER['DOCUMENT_ROOT'].'/rwm/'.$addstring.'/filecreated/'.$filebasename;
...
function vieworgeneratepdfinvoice_backup($slno,$type)
{
	...
	$filebasename = $filename.".pdf";
	$addstring ="/user";
	if($_SERVER['HTTP_HOST'] == "rashmihk" || $_SERVER['HTTP_HOST'] == "meghanab"  || $_SERVER['HTTP_HOST'] == "localhost")
		$addstring = "/saralimax-user";
		$filepath = $_SERVER['DOCUMENT_ROOT'].'/rwm/'.$addstring.'/filecreated/'.$filebasename;
...
function isRegistrationV2Connected($format,$offset,$timestamp){
   global $newconnection;
...
{
	$query="UPDATE inv_mas_scratchcard set registered='" .$status. "' where cardid=".$cardid ;
	$result=runmysqlquery($query);
	return $result;
}
//Added By Bhavesh Patel 05.07.13
function logdetailsregister2($CUSTID,$REGTYPE,$PID,$UNAME,$DWP,$PIN,$HDDID,$ETHID,$UDCID,$CMPNAME,$CMPID,$LGNTYPE)
{
	$BROWSER = $_SERVER['HTTP_USER_AGENT'];
	$SYSTEMIP = $_SERVER['REMOTE_ADDR'];
	$DATETIME = date('Y-m-d')." ".date('h:i:s');
	
	/*$q1 = "SELECT (MAX(slno) + 1) AS newslno FROM inv_logs_register2";
	$r1 = runmysqlquery($q1);
	$f1 = mysql_fetch_array($r1);
	$NEWSLNO = $f1['newslno'];*/
	
	$q2 = "INSERT INTO inv_logs_register2(CUSTID,REGTYPE,PPID,UNAME,PASS,PIN,HDDID,ETHID,UDCID,CMPNAME,CMPID,LGNTYPE,BROWSER,SYSTEMIP,DATES) VALUES ('".$CUSTID."','".$REGTYPE."','".$PID."','".$UNAME."','".$DWP."','".$PIN."','".$HDDID."',
			'".$ETHID."','".$UDCID."','".$CMPNAME."','".$CMPID."','".$LGNTYPE."','".$BROWSER."','".$SYSTEMIP."','".$DATETIME."');";
	$r2 = runmysqlquery($q2);
}
