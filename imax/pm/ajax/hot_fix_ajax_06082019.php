...
switch($submittype)
{
	...
	case "save":
	{
		...
		if($_POST["form_slno"]=="")
		{
			...
			function update_ver($form_slno,$form_patch,$form_product)
			{
				$check_web='0';
				$qry5= "UPDATE prdupdate SET product ='".$form_product."', patchversion ='".$form_patch."', showinweb ='".$check_web."'  
	WHERE slno=".$form_slno;
			...
			if($prdversion == 0)
			{
				...
				$qry3 ="SELECT * FROM prdupdate WHERE product='".$form_product."' AND patchversion='".$form_patch."' AND updatetype = 'hotfix'";
				...
				if($num >= 1)
				{
					while ($row = mysql_fetch_assoc($result3))
					{
						...
						if($last >= $num)
						{
							...
							update_ver($form_slno,$form_patch,$form_product);
						
							$qry1="INSERT INTO prdupdate (product, prdcode, patchversion, size, reldate, patchurl, updatetype, hotfixno, showinweb) values('".$form_product."', '".$form_productcode."', '".$form_patch."', '".$form_filesize."', '".$DPC_date."', '".$form_url."', 'hotfix', '".$form_hotfix."', ".$check_web.")";
		...
		else
		{
			...
			$query3 = "UPDATE prdupdate SET product ='".$form_product."', prdcode ='".$form_productcode."', patchversion ='".$form_patch."', size='".$form_filesize."', reldate ='".$DPC_date."', patchurl ='".$form_url."', updatetype ='hotfix', hotfixno ='".$form_hotfix."', showinweb =".$check_web."
			WHERE slno=".$form_slno;
