...
switch($submittype)
{
	...
	case "saralmail":
	{
		...
		if($command == 'active')
		{
			...
			email_acc_record.createddate ,email_acc_record.employee,
			...
			where email_acc_record.deleted ='NO' and email_acc_record.disable = '0' order by 
			email_acc_record.emailid desc LIMIT 5";
		}
		else if($command == 'disabled')
		{
			...
			from email_acc_record
			left join email_grouphead on  email_grouphead.id = email_acc_record.grouphead
			where email_acc_record.deleted ='NO' and email_acc_record.disable = '1' order by 
			email_acc_record.emailid desc LIMIT 5";
		}
		else if($command =='deleted')
		{
			...
			from email_acc_record
			left join email_grouphead on  email_grouphead.id = email_acc_record.grouphead
			where email_acc_record.deleted ='YES' order by email_acc_record.emailid desc LIMIT 5";
		}
			 
		$grid = '<table width="100%" border="0" bordercolor="#ffffff" cellspacing="0" cellpadding="2" id="gridtablelead" >';
		//Write the header Row of the table
		$grid .= '<tr class="gridheader" style="color:#006699;">
		...
		while($fetch = mysql_fetch_array($result))
		{
			...
			$grid .= '<td nowrap="nowrap" class="tdborderlead">'.changedateformat($fetch['createddate']).'</td>';
	...
	case "flashnews":
	{
		...
		if($command == 'active')
		{
			...
			where disable = 'no' order by slno desc LIMIT 5";
		}
		else if($command == 'disabled')
		{
			$query = "select slno,product,adddeddate ,title,link from saral_flashnews 
			where disable = 'yes' order by slno desc LIMIT 5";
		}
	...
	case "jobcareer":
	{
		...
		if($command == 'activecareer')
		{
			...
			where showinweb = '1' order by slno desc LIMIT 5";
		}
		else if($command == 'disablecareer')
		{
			$query = "select slno,department,experience,qualification,location,createddate from saral_job_required 
			where showinweb = '0' order by slno desc LIMIT 5";
		}
		 ...
		 else
		 {
			 $color = "#f7faff";
			  ...
			  $grid .= '<tr bgcolor='.$color.'>
							...
							<td  class="tdborderlead">'.changedateformat($fetch['createddate']).'</td>
						</tr>';
	...
	{	 
			...
			if($command == 'version')
			{
				$query = "select product,patchversion ,reldate ,verfrom,patchurl from prdupdate 
						where updatetype = 'versionupdate' and  showinweb ='1' order by slno desc LIMIT 5";
			}
			else if($command == 'hotfix')
			{
				$query = "select product,patchversion,reldate,hotfixno,patchurl from prdupdate 
						where updatetype = 'hotfix' and  showinweb ='1' order by slno desc LIMIT 5";
			}
	...
	case "latestactivity":
	{
		...
		,0 as showinweb from saral_flashnews where YEAR(adddeddate)= YEAR(CURDATE()) 
		and disable = 'no') 
		union 
		(select adddeddate as adddeddate ,product as product ,disable as disable , 0 as hotfixno ,
		0 as verfrom , 0 as employee , 0 as deleted , 0 as updatetype ,0 as department 
		, 0 as showinweb from saral_flashnews 
		...
		0 as department , 0 as showinweb  from prdupdate where YEAR(reldate)= YEAR(CURDATE())) 
		union 
		...
		0 as department ,0 as showinweb from email_acc_record 
		...
		0 as verfrom , employee as employee , deleted as deleted , 0 as updatetype ,
		0 as department , 0 as showinweb from email_acc_record 
		where YEAR(disabledate)= YEAR(CURDATE()) and disable = '0') 
		union 
		(select createddate as adddeddate ,0 as product ,0 as disable ,0 as hotfixno ,
		0 as verfrom , 0 as employee , 0 as deleted , 0 as updatetype ,
		department as department , showinweb as showinweb from saral_job_required 
		...
		order by adddeddate desc limit 10";
		...
		$slno = 1;
		/*echo "<br />p_hotfixupdate".$p_hotfixupdate."<br />p_flashnewsupdate".$p_flashnewsupdate;
		...
		while($fetch = mysql_fetch_array($result))
		{
			...
			if($p_hotfixupdate == '1' || $p_versionupdate == '1') 
			{
				##Hotfix update##
				if($updatetype == 'hotfix')
				{
					 $grid .=	'<tr bgcolor='.$color.'>
								 <td class="tdborderlead">'.$slno++.'</td>
								<td class="tdborderlead">Hotfix Added for <b>"'.$fetch['product'].'"</b> on '.changedateformatwithtime($fetch['adddeddate']). '</td>
								</tr>';
				}
				##version update##
				else if($updatetype == 'versionupdate')
				{
				  $grid .= '<tr bgcolor='.$color.'>
								 <td class="tdborderlead">'.$slno++.'</td>
								<td class="tdborderlead">Version Update Added for <b>"'.$fetch['product'].'"</b> on '.changedateformatwithtime($fetch['adddeddate']). '</td>
							</tr>';
				}
			}
			
			else if($p_saralmail_delete == '1' || $p_saralmail == '1' || $p_saralmail_disable == '1') 
			{
				...
				if($deleted == 'YES')
				{
				  $grid .= '<tr bgcolor='.$color.'>
								 <td class="tdborderlead">'.$slno++.'</td>
								<td  class="tdborderlead">Official Email ID Deleted for <b>"'.$fetch['employee'].'"</b> on '.changedateformatwithtime($fetch['adddeddate']). '</td>
							</tr>';
				}
				...
				else if($deleted == 'NO')
				{
				  $grid .= '<tr bgcolor='.$color.'>
								 <td class="tdborderlead">'.$slno++.'</td>
								<td  class="tdborderlead">Official Email ID Created for <b>"'.$fetch['employee'].'"</b> on '.changedateformatwithtime($fetch['adddeddate']). '</td>
							</tr>';
				}
				...
				if($disable == '0')
				{
				  $grid .= '<tr bgcolor='.$color.'>
								 <td class="tdborderlead">'.$slno++.'</td>
								<td  class="tdborderlead">Official Email ID Enabled for <b>"'.$fetch['employee'].'"</b> on '.changedateformatwithtime($fetch['adddeddate']). '</td>
							</tr>';
				}
				...
				else if($disable == '1')
				{
				  $grid .= '<tr bgcolor='.$color.'>
								 <td class="tdborderlead">'.$slno++.'</td>
								<td  class="tdborderlead">Official Email ID Disabled for <b>"'.$fetch['employee'].'"</b> on '.changedateformatwithtime($fetch['adddeddate']). '</td>
							</tr>';
				}
			}
			else if($p_career == '1') 
			{
				...
				if($showinweb == '1')
				{
				  $grid .= '<tr bgcolor='.$color.'>
								 <td class="tdborderlead">'.$slno++.'</td>
								<td  class="tdborderlead">Job Openings Available for <b>"'.$fetch['department'].'"</b> on '.changedateformatwithtime($fetch['adddeddate']). '</td>
							</tr>';
				}
				...
				else if($showinweb == '0')
				{
				  $grid .= '<tr bgcolor='.$color.'>
								 <td class="tdborderlead">'.$slno++.'</td>
								<td  class="tdborderlead">Job Openings Un-Available for <b>"'.$fetch['department'].'"</b> on '.changedateformatwithtime($fetch['adddeddate']). '</td>
							</tr>';
				}
			}
			else if($p_flashnewsupdate == '1') 
			{
				##Flash News Disable##
				if($disable == 'yes')
				{
				  $grid .=	'<tr bgcolor='.$color.'>
							  <td class="tdborderlead">'.$slno++.'</td>
								<td class="tdborderlead">Flash News Disabled for <b>"'.$fetch['product'].'"</b> on '.changedateformatwithtime($fetch['adddeddate']). '</td>
							</tr>';
				}
				##Flash News Active##
				else if($disable == 'no')
				{
				  $grid .=	'<tr bgcolor='.$color.'>
								 <td class="tdborderlead">'.$slno++.'</td>
								<td class="tdborderlead">Flash News Added for <b>"'.$fetch['product'].'"</b> on '.changedateformatwithtime($fetch['adddeddate']). '</td>
							</tr>';
				}
			}
		}
