...
switch($submittype)
{
	...
	case "table":
	{
		...
		else
		{
			...
			$query = "SELECT prdcode,patchversion,size ,reldate,verfrom,patchurl,showinweb,slno FROM prdupdate WHERE updatetype = 'versionupdate' AND product = '".$form_product."' 
			order by slno desc ";
	...
	case "save":
	{
		...
		if($form_slno=="")
		{
				...
				function update_ver($form_slno,$form_patch,$form_product,$form_productcode,$form_filesize,$DPC_date,$form_url)
				{
					$web='0';
					$qry5= "UPDATE prdupdate SET product ='".$form_product."', prdcode ='".$form_productcode."', patchversion ='".$form_patch."', size='".$form_filesize."', reldate ='".$DPC_date."', patchurl ='".$form_url."', showinweb ='".$web."'  
		WHERE slno=".$form_slno;
				...
				$qry3 ="SELECT * FROM prdupdate WHERE product='".$form_product."' AND verfrom BETWEEN '".$form_verfrom."' AND '".$form_patch."'";
				...
				if($num >= 1)
				{
					while ($row = mysqli_fetch_assoc($result3))
					{
						...
						if($last >= $num)
						{
							...
							update_ver($form_slno,$form_patch,$form_product,$form_productcode,$form_filesize,$DPC_date,$form_url);
						
							$qry1="INSERT INTO prdupdate (product, prdcode, patchversion, size, reldate, verfrom, patchurl, updatetype, applicable, showinweb) values('".$form_product."', '".$form_productcode."', '".$form_patch."', '".$form_filesize."', '".$DPC_date."', '".$lastvariable."', '".$form_url."', 'versionupdate', '".$form_verfrom."', ".$show_web.")";
				...
				{
					$query2 = "INSERT INTO prdupdate (product, prdcode, patchversion, size, reldate, verfrom, patchurl, updatetype, applicable, showinweb) values('".$form_product."', '".$form_productcode."', '".$form_patch."', '".$form_filesize."', '".$DPC_date."', '".$form_verfrom."', '".$form_url."', 'versionupdate', '".$form_verfrom."', ".$show_web.")";
		...
		else
		{
				...
				$query3 = "UPDATE prdupdate SET product ='".$form_product."', prdcode ='".$form_productcode."', patchversion ='".$form_patch."', size='".$form_filesize."', reldate ='".$DPC_date."', verfrom ='".$form_verfrom."', patchurl ='".$form_url."', updatetype ='versionupdate', applicable='".$form_verfrom."', showinweb =".$show_web."  
		WHERE slno=".$form_slno;
...
select  patchversion as patch_verfrom from prdupdate where product='".$form_product."' and patchversion <>''ORDER BY patch_verfrom desc";
	...
	case "gridtoform" :
	{
		...
		if($fetch1['count'] > 0)   
		{
			...
			echo('1^'.$fetch['slno'].'^'.$fetch['product'].'^'.$fetch['prdcode'].'^'.$fetch['patchversion'].'^'.
